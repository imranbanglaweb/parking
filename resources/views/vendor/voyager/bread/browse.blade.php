...
@section('page_title', __($dataType->display_name_plural))
                            ...
                            <tr>
                                @foreach($dataType->browseRows as $row)
                                    ...
                                    <td>
                                        ...
                                        @elseif (isset($row->additional_details) && isset($row->additional_details->browse_view))
                                            @include($row->additional_details->browse_view, ['row' => $row, 'dataType' => $dataType, 'dataTypeContent' => $dataTypeContent, 'content' => $data, 'action' => 'browse'])
                                        ...
                                        @elseif($row->type == 'date')
                                            {{ property_exists($row->details, 'format') ? \Carbon\Carbon::parse($data->{$row->field})->format($row->details->format) : $data->{$row->field} }}
                                        @elseif($row->type == 'timestamp')
                                            {{ property_exists($row->details, 'format') ? \Carbon\Carbon::parse($data->{$row->field})->format($row->details->format) : $data->{$row->field} }}
                                        @elseif($row->type == 'time' )
                                            {{ property_exists($row->details, 'format') ? \Carbon\Carbon::parse($data->{$row->field})->format($row->details->format) :\Carbon\Carbon::parse($data->{$row->field})->format("h:i:s a") }}
