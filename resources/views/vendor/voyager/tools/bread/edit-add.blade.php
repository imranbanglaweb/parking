        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <form action="@if(isset($dataType->id)){{ route('voyager.bread.update', $dataType->id) }}@else{{ route('voyager.bread.store') }}@endif"
                      method="POST" role="form" id="tool-bread-add-edit-form">
                @if(isset($dataType->id))
                ...
                @endif
                    ...
                    <div class="panel panel-primary panel-bordered">
                        ...
                        <div class="panel-body">
                            ...
                            <div class="row clearfix">
                                ...
                                <div class="col-md-9">
                                    <div class="additional_permission_container">
                                        <button id="add_additional_permission" type="button" class="btn btn-default" title="Click Here to add additional permissions">{{ __('voyager::bread.add_additional_permissions') }}</button>
                                        <ul class="additional_permission_box" id="additional_permission_box">
                                            <?php if(!empty($additionalPermissions) && isset($dataType) && !empty($dataType->name)): ?>
                                                <?php foreach($additionalPermissions as $additionalPermission): ?>
                                                <?php $additionalPermissionCode = str_replace('_'.$dataType->name, '', $additionalPermission->key); ?>
                                                <li><input type='text' name='additional_permissions[]' class='additional_permission_input' value="{{$additionalPermissionCode}}">&nbsp;<span class='rem_additional_permission'>X</span></li>
                                                <?php endforeach ?>
                                            <?php endif; ?>
                                        </ul>
                                    </div>
                            ...
                            @foreach($fieldOptions as $data)
                                ...
                                <div class="row row-dd">
                                    ...
                                    <div class="col-sm-2">
                                        ...
                                        <label for="field_search_{{ $data['field'] }}">{{ __('voyager::generic.search') }}</label><br/>
                                    </div>
                                    <div class="col-sm-2">
                                    ...
                                    <div class="col-sm-4">
                                        ...
                                        <textarea id="json-input-{{ json_encode($data['field']) }}" class="ace-editor-textarea resizable-editor" data-editor="json" name="field_details_{{ $data['field'] }}">@if(isset($dataRow->details)){{ json_encode($dataRow->details) }}@else{}@endif</textarea>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <button type="button" id="submitandcontinuebtn" class="btn pull-right btn-primary" style="margin-right: 20px;">{{ __('voyager::generic.submitandcontinue') }}</button>
                        </div>
                    </div>
                </form>
            </div><!-- .col-md-12 -->
...
@section('javascript')
    <script type="text/javascript" src="{{ asset('/backend_resources/js/form-with-editor.js') }}"></script>
    ...
    <script>
        ...
        $(function () {
            ...
            $('#submitandcontinuebtn').on('click', function(e){
                var form = $('#tool-bread-add-edit-form');

                var action = form.prop('action');

                form.prop('action', action + "?continue=1").submit();
            });
            /**
             * Reorder items
             */
            ...
            /** handle additional permission codes */

            $('#add_additional_permission').click(function (e) {
                $('#additional_permission_box').append("<li><input type='text' name='additional_permissions[]' class='additional_permission_input' >&nbsp;<span class='rem_additional_permission'>X</span></li>");
            });

            $(document).on('click', '.rem_additional_permission', function (e) {
              $(this).prev('.additional_permission_input').remove();
              $(this).hide();
            });
        });
...
@section('css')
    <style>
        .additional_permission_container{
            width: 100%;
            height: auto;
            min-height: 50px;
            border: 1px solid #f1f1f1;
        }
        .additional_permission_box {
            list-style-type: none;
        }
        .additional_permission_box li {
            display: inline-block;
            margin-left: 10px;
            margin-top: 10px;
        }
        .rem_additional_permission {
            display: inline-block;
            font-size: 12px;
            font-weight: bold;
            cursor: pointer;
